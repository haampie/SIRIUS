...
# Bundle things -- this is a bit more complicated than it should unfortunately :(
# We wanna run `ctest`, but ctest does not work well after running make install
# So instead we just don't install, but prune the build folder as much as possible.
# We move the libs to `/root/sirius.bundle/lib` and also copy `ctest` to `/root/sirius.bundle/ctest`.
